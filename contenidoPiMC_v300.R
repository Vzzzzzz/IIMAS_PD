...
# Selección de número de variables de entrada X_i.
opciones_mv_1 <- tagList(
  selectizeInput(inputId = "nVariables",
                 label = "Número de variables de entrada X",
                 choices = seq(from = 1, to = 6, by = 1),
                 options = list(search = F)) |>
    tooltip("Selecciona la cantidad de variables de entrada X",
            placement = "right")
  )

# Selección de número de variables de entrada Y_i.
opciones_mv2_1 <- tagList(
  selectizeInput(inputId = "nVariablesY",
                 label = "Número de variables de entrada X",
                 choices = seq(from = 1, to = 6, by = 1),
                 options = list(search = F)) |>
    tooltip("Selecciona la cantidad de variables de entrada X",
...
opciones_mv2_2 <- tagList(
  selectizeInput(inputId = "mVariables",
                 label = "Número de variables de salida Y",
                 choices = seq(from = 2, to = 6, by = 1),
                 options = list(search = FALSE)) |>
    tooltip("Selecciona la cantidad de valores de salida", placement = "right"),
  uiOutput("transformacion_F"),
  tags$hr()
)
...
# función recuadros de texto para transformaciones Y_i
funcion_transformacion_salida <- function(i, input) {tagList(
  { paste0("Transformación f_", i) },
  textAreaInput(inputId = sprintf("transformacion_f%d", i),
                label = NULL,
                value = "x_1^2",
                resize = "vertical") |>
    tooltip("Escribe la transformación para aplicar a las variable de entrada X_i
...
tabla_cobertura_dinamica <- function(df, cobertura) {
  proba_min <- (1 - cobertura) / 2
  proba_max <- 1 - proba_min
  df_cobertura <- data.frame("..." = c("Intervalo", "Factor de cobertura"))
  for (Xi in names(df)) {
    int_min <- round(quantile(df[[Xi]], probs = proba_min), 2)
    int_max <- round(quantile(df[[Xi]], probs = proba_max), 2)
    k <- round((int_max - mean(df[[Xi]])) / sd(df[[Xi]]), 2)
    df_cobertura <- cbind(
      df_cobertura, 
      setNames(data.frame(Xi = c( paste0("(", int_min, ", ", int_max, ")"), k)),
               Xi)
          )
  }
  return(df_cobertura)
...
panel_pagina_2 <- tagList(
  card(fill = FALSE,
       ...
       card_body(
         ...
         #### T A B L A   S U M A R I O.   #################################
         div(
           div(class = "card-header bg-light text-dark mb-1", "Estadísticas"),
           div(style = "overflow-x: auto;", tableOutput("tabla_sumario_mv"))
         ),
         #### T A B L A   C O B E R T U R A.   #############################
         div(
           div(class = "card-header bg-light text-dark mb-1", "Intervalos de cobertura"),
           div(style = "overflow-x: auto;", tableOutput("tabla_cobertura_mv"))
         ),
         #### T A B L A   C O B E R T U R A   I N T E R A C T I V A.   #####
         div(
           div(class = "card-header bg-dark text-white mb-3", "Tabla de cobertura interactiva"),
           numericInput(inputId = "cobertura_interactiva_mv",
                        label = NULL,
                        value = 85,
                        min = 68,
                        max = 99, 
                        step = 1), 
           div(style = "overflow-x: auto;", tableOutput("tabla_cobertura_interactiva_mv"))
         ),
...
# Panel de resultados Y_i.
panel_pagina_3 <- tagList(
  card(fill = FALSE,
       class = "border-primary",
       card_body(
         layout_columns(
           ...
           card(
             fill = FALSE,
             card_header(class = "bg-dark", "Histogramas variables de salida Y"),
             class = "border-secondary",
             full_screen = TRUE,
             height = "400px",
             card_body( plotOutput("grafica_combinada_mv2_Y"))
             ),
           # Histogramas X_i.
           card(
             fill = FALSE,
             card_header(class = "bg-dark", "Histogramas variables de entrada X"),
             class = "border-secondary",
             full_screen = TRUE,
             height = "400px",
             card_body(plotOutput("grafica_combinada_mv2_X"))
             ),
           # Tabla sumario Y_i.
           div(
             div(class = "card-header bg-light border-light text-dark mb-1", "Estadísticas de Y"),
             div(style = "overflow-x: auto;", tableOutput("tabla_sumario_mv2_Y"))
           ),
           # Tabla sumario X_i.
           div(
             div(class = "card-header bg-light text-dark mb-1", "Estadísticas de X"),
             div(style = "overflow-x: auto;", tableOutput("tabla_sumario_mv2_X"))
           ),
           # Tabla intervalos cobertura Y_i.
           div(
             div(class = "card-header bg-light text-dark mb-1", "Intervalos de cobertura Y"),
             div(style = "overflow-x: auto;", tableOutput("tabla_cobertura_mv2_Y"))
           ),
           # Tabla intervalos cobertura X_i.
           div(
             div(class = "card-header bg-light text-dark mb-1", "Intervalos de cobertura X"),
             div(style = "overflow-x: auto;", tableOutput("tabla_cobertura_mv2_X"))
           )),
         # Tabla interactiva cobertura mv2.
         div(
           div(class = "card-header bg-dark text-white mb-3", "Tabla de cobertura interactiva"),
           numericInput(inputId = "cobertura_interactiva_mv2",
                          label = NULL,
                          value = 85,
                          min = 68,
                          max = 99, 
                          step = 1), 
             div(style = "overflow-x: auto;", tableOutput("tabla_cobertura_interactiva_mv2"))
           ),
         layout_columns(
           col_widths = c(6,6),
           ...
           card(fill = FALSE,
                ...
                height = "400px",
                card_body(
                  plotOutput("heatmap_pearson_mv2")
                )),
           ...
           card(fill = FALSE,
                ...
                height = "400px",
                card_body(
                  plotOutput("heatmap_spearman_mv2")
                )),
           div(style = "overflow-x: auto;", tableOutput("correlacion_pearson_mv2")),
           div(style = "overflow-x: auto;", tableOutput("correlacion_spearman_mv2"))
           )
       ))
  )
